...
def save(outdir: Path, cfg: Config, runs: list[dict], summary: dict, report: str) -> None:
    ...
    try:
        import sys
        sys.path.insert(0, str(ROOT.parents[1] / "tools" / "result-provenance"))
        import provenance
        (outdir / "provenance.json").write_text(json.dumps(provenance.stamp(
            ["tools/universal-wave-translation-layer"]), indent=1, default=str))
