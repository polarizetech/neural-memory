...
def cmd_exp(a) -> None:
    ...
    table = dict(delay=suite.exp_delay, streams=suite.exp_streams, ablations=suite.exp_ablations,
                 ...
                 recall_modes=suite.exp_recall_modes, recall_drive=suite.exp_recall_drive, binaural=suite.exp_binaural, completion=suite.exp_completion, storage_diagnostic=diagnostic.exp_storage_diagnostic, residual_decode=diagnostic2.exp_residual_decode, storage_A=diagnostic2.exp_storage_A, storage_B=diagnostic2.exp_storage_B, repro_lt2021=repro_lt2021.exp_repro_lt2021)
    names = [n for n in table if n not in ("recall_drive", "binaural", "completion", "storage_diagnostic", "residual_decode", "storage_A", "storage_B", "repro_lt2021")] if a.name == "all" else [a.name]
    if a.seeds < 10:
        print(f"NOTE: {a.seeds} seeds requested; the reporting standard for this project is >= 10.")
    for n in names:
